...
A bejövő adatnak a spektrumát ismerünk kell a pszichoakusztív modell alkalkazásához, ezért FFT-t hajtunk végre a bejövő jelen, a filterbankkal párhuzamosan.
...
A SAMPLE\_IN állapotban írjuk a bejövő mintákat a BRAM-ba, számlálókkal vannak számon tartva, hogy mennyi jött be (külön számlálók a 256 és 1024 pontos FFT-hez), és amint van elegendő egy FFT elvégzéséhez, akkor átmegyünk a CONFIG állapotba.
Itt megvizsgáljuk, hogy melyik számláló hatására léptünk ide, és konfiguráljuk az FFT core-t. Ezután vagy a SMALL\_BLOCK\_READ vagy a LARGE\_BLOCK\_READ állapotba lépünk, és elvégezzük az adatátvitelt az IP core bemenetére, majd visszatérünk a SAMPLE\_IN állapotba. 
...
Például ha a SMALL\_BLOCK\_READ állapotban vagyunk, akkor 256 kiolvasott minta után a small\_read\_offset változót 192-vel növeljük.
...
A Vivado-ban készített szimuláció waveformja a(z) \ref{fig:sim}. ábrán látható. A tesztekhez kezdetben egy Radix-2-lite Burst I/O architektúrásjú, fixed-point számábrázolású, real-time, natural order kimenetű FFT-t használunk. A végleges megoldásban bit/digit reversed order lesz, azonban még a tesztelésnél segít, ha natural order van. A bemeneti adatszélesség 16 bit, a core 16bit valós és 16 bit képzetes részt vár, a képzetes részre 0-kat konkatenálunk.
...
1kHz-s szinusz jel a bemenet, we\_a\_reg a 44.1kHz-s mintavételi engedélyező jel, ahol pedig fft\_out\_valid tüskét látunk, ott van FFT kimenetünk.
...
\begin{itemize}
    ...
    \item \textbf{T4:} A szorzat érvényessé válik a kimeneten, a \texttt{data\_in\_valid[4]} az FFT-nek a T\_VALID jele.
\end{itemize}

Az IP core az első vétel után  a T\_READY jelet lehúzza, amíg készen nem áll a feldolgozásra, addig az első adatot egy buffer-ban tárolja, ez látszik a(z) \ref{fig:bram_read}. ábrán is.

Vizsgáljuk meg, hogy mennyi időbe telik egy 256 és egy 1024 pontos FFT végrehajtása. A data\_in\_valid[4] felfutó élétől az fft\_out\_valid lefutó éléig mérjük az időt.
...
A(z) \ref{fig:1024_FFT} .ábrán látszik, hogy a 256-os FFT után amíg átkonfigurálódik az IP core 1024-pontosra, addig lehúzza a data\_in\_ready-t, és csak utána folytatódik a feldolgozás.

Az ábrák alapján az N=256-os FFT 13.37 \unit{\us} alatt megy végbe, az N=1024-es 74.2 \unit{\us} alatt. Ez alapján bőven elég egy darab Radix-2-lite Burst I/O architektúrájú FFT IP core időmultiplexáltan használva.
...
Mivel 200MHz-es az órajelünk, ezért az órajelperiódus T=5ns. Az ábrán mért időkből egy periódust le kell vonni, mert jelenleg az abszolútérték képzés egy órajelbe telik.
...
Ha a felbontásnak nem a kerekített értékével szoroznánk, akkor mégjobban visszaadná a számított frekvencia az eredeti jelét, de így is látszik, hogy az történik amire számítunk.
...
A félév folyamán megismerkedtem az audio tömörítő codec-ek világával, mélyebben az MPEG1-Audio Layer III (MP3)-al. Mivel ebben a félévben kezdtem a témát, ezért jelentős mennyiségű idő irodalomkutatással telt. HDL kódban megvalósításra a bejövő jel szabvány szerinti FFT-zése került. 
